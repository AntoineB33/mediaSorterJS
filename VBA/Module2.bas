...
Public thumbnailHeight As Integer
...
Sub DisplayImagesInUserForm()
    ...
    Do While count < maxCount And Not (currentNum = minInt And Not decrease) And Not (currentNum = maxInt And decrease)
        ...
        If decrease Then
            ...
            If currentNum <= minInt Then
                decrease = False
            End If
        Else
            currentNum = currentNum + 1
            If currentNum >= maxInt Then
                decrease = True
            End If
        End If
    Loop
    ...
    For i = 1 To count
        ...
        If FilePath <> "" Then
            ...
            If IsImageFile(FilePath) Then
                Set imgControl.Picture = LoadPicture(FilePath)
                imgControl.BorderColor = RGB(255, 255, 0)
            ElseIf IsVideoFile(FilePath) Then
                ' If it's a video, extract a frame
                Set imgControl.Picture = ExtractFrameFromVideo(FilePath, FileNameWithoutExt)
                imgControl.BorderColor = RGB(255, 0, 0)
            End If
            ...
            With imgControl
                ...
                .SpecialEffect = fmSpecialEffectFlat ' Flat border
                .BorderStyle = fmBorderStyleSingle ' Single-line border
                .BorderWidth = 3 ' Set the thickness of the border
            End With
        Else
        ...
        If selectedThumbnail Then
            imgControl.BorderColor = RGB(0, 0, 0)
            selectedThumbnail = False
        End If
    Next i
